...
import com.capstone1.findable.oauth.service.RedisRefreshTokenService;
...
//    private final RefreshTokenRepo refreshTokenRepo;
    ...
    public ResponseEntity<Map<String, String>> refreshAccessToken(@RequestBody RefreshTokenRequest request) {
        ...
        if (jwtTokenProvider.isTokenBlacklisted(refreshToken)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "Refresh Token is blacklisted."));
        }

        // Redis에서 Refresh Token 확인
        String username = jwtTokenProvider.getUsernameFromToken(refreshToken);
        String storedToken = redisRefreshTokenService.getRefreshToken(username);
        ...
        redisRefreshTokenService.saveRefreshToken(username, newRefreshToken);
...
//        User user = jwtTokenProvider.getUserFromToken(refreshToken); // 토큰에서 사용자 정보 가져오기
//        String newAccessToken = jwtTokenProvider.generateAccessToken(user.getUsername());
//        String newRefreshToken = jwtTokenProvider.generateRefreshToken(user.getUsername());

//        // Redis에 새로운 Refresh Token 저장
//        redisRefreshTokenService.saveRefreshToken(user.getUsername(), newRefreshToken);
//
...
//        ));
    ...
    public ResponseEntity<String> logout(@RequestBody RefreshTokenRequest request) {
        ...
        String username = jwtTokenProvider.getUsernameFromToken(refreshToken);
        if (username == null || redisRefreshTokenService.getRefreshToken(username) == null) {
            logger.warn("⚠️ Invalid Refresh Token during logout.");
        ...
        blacklistedTokenRepo.save(BlacklistedToken.builder()
                ...
                .build());
        logger.info("Refresh Token added to blacklist and removed from Redis: {}", refreshToken);
